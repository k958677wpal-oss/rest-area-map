...
def normalize_value(value):
    ...
    - [핵심] 사번처럼 '00501580'이 시트에서 숫자로 읽혀 앞자리 0이 사라지는 문제를
      해결하기 위해, 숫자로만 이루어진 값은 양쪽 모두 앞자리 0을 떼고(정수화) 비교합니다.
    """
    if value is None:
        return ""
    ...
    s = s.strip()
    s = s.replace("\u00a0", "").replace("\u200b", "")
    # [핵심] 숫자로만 이루어진 값이면 앞자리 0을 제거해 비교 기준을 통일
    #  예) 입력 '00507583' → '507583', 시트 '507583' → '507583' → 서로 일치
    if s.isdigit():
        s = str(int(s))
    return s
